...
mainEM_orCloudy <- function(method){
  x <- foreach(f = list.files(pattern = ".csv"), .combine = c,  .export = c("getResultList")) %:% 
    foreach(t = 1:3) %do% {
      setwd("D:/~Masters/~ MS-STAT/~THESIS/Code/codes for simulation/3 - simulation/simulated3/")
      source("../getEquallyLogSpacedIntervals.R")
      source("D:/~Masters/~ MS-STAT/~THESIS/Papers/(Supplementary Files) Jacobs/umbrella-master/1D/Umbrella_1d_V1.R")
      ...
      if(method == "EM"){
        ...
        emres <- DropEmPCR(target, volDrp = 0.85, distr = "mvt")
        classification <- emres$classification[order(target)]
      }else if(method == "DropEmPCR_tskew"){
        emres <- DropEmPCR(target, volDrp = 0.85, distr = "mst")
        classification <- emres$classification[order(target)]
